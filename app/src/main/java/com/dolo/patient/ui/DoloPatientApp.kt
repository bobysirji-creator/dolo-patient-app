...
object Routes{const val Splash="splash";const val Login="login";const val Home="home";const val Categories="categories";const val Doctors="doctors/{category}";const val DoctorDetails="doctor/{doctorId}";const val Booking="booking/{doctorId}";const val Confirmation="confirmation/{doctorId}/{session}";const val History="history";const val Favourites="favourites"}
@Composable fun DoloPatientApp(authRepository:AuthRepository,patientRepository:PatientRepository){
 ...
 NavHost(nav,startDestination=Routes.Splash){
  ...
  composable(Routes.Home){HomeScreen(onCategories={nav.navigate(Routes.Categories)},onDoctor={nav.navigate("doctor/"+it)},onHistory={nav.navigate(Routes.History)},onFavourites={nav.navigate(Routes.Favourites)},onLogout={auth.logout();nav.navigate(Routes.Login){popUpTo(Routes.Home){inclusive=true}}},state=patient.uiState,onSearch={patient.search(it);nav.navigate("doctors/All")})}
  ...
  composable(Routes.History){AppointmentHistoryScreen(patient.uiState.appointments,nav::popBackStack)}
