...
@Composable fun DoloPatientApp(authRepository:AuthRepository,patientRepository:PatientRepository){
 ...
 NavHost(nav,startDestination=Routes.Splash){
  ...
  composable(Routes.Home){HomeScreen(onCategories={nav.navigate(Routes.Categories)},onDoctor={nav.navigate("doctor/"+it)},onHistory={nav.navigate(Routes.History)},onFavourites={nav.navigate(Routes.Favourites)},onQueue={patient.uiState.active?.id?.let{nav.navigate("queue/"+it)}},onProfile={nav.navigate(Routes.Profile)},onNotifications={nav.navigate(Routes.Notifications)},onSupport={nav.navigate(Routes.Support)},onLogout={auth.logout();nav.navigate(Routes.Login){popUpTo(Routes.Home){inclusive=true}}},state=patient.uiState,onSearch={patient.search(it);nav.navigate("doctors/All")})}
  composable(Routes.Categories){CategoriesScreen(nav::popBackStack){nav.navigate("doctors/"+it)}}
  composable(Routes.Doctors,arguments=listOf(navArgument("category"){type=NavType.StringType})){e->val c=e.arguments?.getString("category").orEmpty();DoctorListScreen(c,nav::popBackStack,patient.uiState,{patient.search(it,if(c=="All")null else c)},{nav.navigate("doctor/"+it)},patient::toggleFavourite,{nav.navigate(Routes.Home){launchSingleTop=true}},{nav.navigate(Routes.History){launchSingleTop=true}},{nav.navigate(Routes.Categories){launchSingleTop=true}})}
  ...
  composable(Routes.Integrations){IntegrationStatusScreen(nav::popBackStack)}
