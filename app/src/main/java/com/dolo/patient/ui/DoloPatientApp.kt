...
@Composable fun DoloPatientApp(authRepository:AuthRepository,patientRepository:PatientRepository,platformApi:PlatformApi,hostedSyncApi:HostedPatientSyncApi){
 ...
 NavHost(
  ...
  composable(Routes.Home){
   ...
   PatientHomeRoute(patientState=patient.uiState,hostedState=if(hostedMode)hosted.uiState else null,onSearchDoctors={patient.search("");nav.navigate("doctors/All")},onNearMe={patient.search("");nav.navigate("doctors/All")},onAllQueues={nav.navigate(Routes.AllQueues)},onQueue={nav.navigate("queue/"+it)},onNotifications={nav.navigate(Routes.Notifications)},onFavorites={nav.navigate(Routes.Favourites)},onDoctor={nav.navigate("doctor/"+it)},onBookDoctor={nav.navigate("booking/"+it)},onAppointments={nav.navigate(Routes.History)},onBook={nav.navigate(Routes.Categories)},onHistory={nav.navigate(Routes.History)},onProfile={nav.navigate(Routes.Profile)},onSupport={nav.navigate(Routes.Support)},onLogout={auth.logout();nav.navigate(Routes.Login){popUpTo(Routes.Home){inclusive=true}}},onRefreshQueues=patient::refreshAllQueues,onRefreshHosted=hosted::refresh,onHostedSync={nav.navigate(Routes.HostedSync)})
  }
  composable(Routes.AllQueues){AllQueuesScreen(patient.uiState,if(auth.uiState.session?.mode==PatientAuthMode.HOSTED_PROTOTYPE)hosted.uiState else null,nav::popBackStack){nav.navigate("queue/"+it)}}
  ...
  composable(Routes.History){AppointmentHistoryScreen(patient.uiState.appointments,nav::popBackStack,{nav.navigate("queue/"+it)},{patient.reschedule(it)},{doctorId,appointmentId->nav.navigate("review/"+doctorId+"/"+appointmentId)},patient::canReschedule,patient::canReview,{nav.openPrimary(Routes.Home)},{nav.openPrimary(Routes.Categories)})}
  ...
  composable(Routes.Profile){ProfileScreen(patient.uiState,auth.uiState.identityCard,auth.uiState.identityMessage,auth::refreshIdentityCard,nav::popBackStack,{name,phone,city->patient.updateProfile(name,phone,city)},{name,relation,age->patient.addFamilyMember(name,relation,age)})}
