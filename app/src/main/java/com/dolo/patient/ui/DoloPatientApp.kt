...
@Composable fun DoloPatientApp(authRepository:AuthRepository,patientRepository:PatientRepository){
 ...
 NavHost(nav,startDestination=Routes.Splash){
  ...
  composable(Routes.History){AppointmentHistoryScreen(patient.uiState.appointments,nav::popBackStack,{nav.navigate("queue/"+it)},{patient.reschedule(it)},{doctorId,appointmentId->nav.navigate("review/"+doctorId+"/"+appointmentId)},patient::canReschedule,patient::canReview)}
