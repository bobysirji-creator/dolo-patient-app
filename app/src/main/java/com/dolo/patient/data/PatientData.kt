...
private val ACTIVE_APPOINTMENT_STATUSES = setOf(
...
object QueueCalculator{
 ...
 fun snapshot(
  ...
  currentTokenStartedAt:Long=refreshedAt
 ):QueueSnapshot{
  val current=currentToken.coerceIn(1,a.token)
  val ahead=(a.token-current-1).coerceAtLeast(0)
  val waitTokens=(a.token-current).coerceAtLeast(0)
  ...
  return QueueSnapshot(a.id,a.token,current,ahead,waitTokens*AVERAGE_CONSULTATION_MINUTES,status,refreshedAt,currentTokenStartedAt)
 }
}
object QueueCountdown{
 fun remainingSeconds(snapshot:QueueSnapshot?,now:Long=System.currentTimeMillis()):Long{
  if(snapshot==null||snapshot.status!=AppointmentStatus.WAITING)return 0
  val elapsed=((now-snapshot.currentTokenStartedAt).coerceAtLeast(0L))/1000L
  return (snapshot.estimatedMinutes*60L-elapsed).coerceAtLeast(0L)
 }
 fun format(seconds:Long):String{
  ...
  val ss=remaining.toString().padStart(2,'0')
  return if(hours>0)hours.toString()+":"+mm+":"+ss else mm+":"+ss
 }
}
...
class LocalPatientRepository(private val preferences:SharedPreferences):PatientRepository{
 ...
 override fun activeAppointment()=appointments().firstOrNull{it.status in ACTIVE_APPOINTMENT_STATUSES}
 ...
 override fun queue(appointmentId:String):QueueSnapshot?{
  val a=appointments().firstOrNull{it.id==appointmentId}?:return null
  val now=System.currentTimeMillis()
  val initial=(a.token-5).coerceAtLeast(1)
  val current=preferences.getInt(queueKey(appointmentId),initial)
  val refreshed=preferences.getLong(refreshKey(appointmentId),now)
  var started=preferences.getLong(queueStartedKey(appointmentId),0L)
  if(started<=0L){
   started=now
   preferences.edit().putLong(queueStartedKey(appointmentId),started).apply()
  }
  return QueueCalculator.snapshot(a,current,refreshed,started)
 }
 override fun advanceQueue(appointmentId:String):QueueSnapshot?{
  val a=appointments().firstOrNull{it.id==appointmentId}?:return null
  if(a.status==AppointmentStatus.MISSED)return queue(appointmentId)
  val next=((queue(appointmentId)?.currentToken?:1)+1).coerceAtMost(a.token)
  val now=System.currentTimeMillis()
  preferences.edit().putInt(queueKey(appointmentId),next).putLong(refreshKey(appointmentId),now).putLong(queueStartedKey(appointmentId),now).apply()
  val updated=a.copy(status=if(next>=a.token)AppointmentStatus.IN_CONSULTATION else AppointmentStatus.WAITING)
  save(updated)
  val snapshot=QueueCalculator.snapshot(updated,next,now,now)
  if(snapshot.patientsAhead<=2)notify("Your turn is approaching",snapshot.patientsAhead.toString()+" patients ahead")
  return snapshot
 }
...
data class PatientUiState(
    ...
    val queues: Map<String, QueueSnapshot> = emptyMap(),
...
class PatientViewModel(private val repository: PatientRepository) : ViewModel() {
    ...
    fun refreshAllQueues() {
        uiState.appointments
            .filter { it.status in ACTIVE_APPOINTMENT_STATUSES }
            .forEach { repository.refreshQueue(it.id, true) }
        refresh(syncStatus = SyncStatus.FRESH)
    }
    ...
    private fun refresh(
        ...
        val activeAppointments = appointments.filter { it.status in ACTIVE_APPOINTMENT_STATUSES }
        val active = activeAppointments.firstOrNull()
        val queueId = id ?: active?.id
        val queues = activeAppointments.mapNotNull { appointment ->
            val snapshot = if (appointment.id == id && queueOverride != null) {
                queueOverride
            } else {
                repository.queue(appointment.id)
            }
            snapshot?.let { appointment.id to it }
        }.toMap()
        uiState = uiState.copy(
            appointments = appointments,
            active = active,
            queue = queueId?.let(queues::get),
            queues = queues,
    ...
    private fun loadState(): PatientUiState {
        ...
        val active = activeAppointments.firstOrNull()
        val queues = activeAppointments.mapNotNull { appointment ->
            repository.queue(appointment.id)?.let { appointment.id to it }
        }.toMap()
        return PatientUiState(
            appointments = appointments,
            active = active,
            favouriteIds = repository.favouriteDoctorIds(),
            queue = active?.id?.let(queues::get),
            queues = queues,
            profile = repository.profile(),
            family = repository.familyMembers(),
            reviews = repository.reviews(),
