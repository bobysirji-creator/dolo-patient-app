...
## Stage 60A implementation checkpoint
...
Patient App `0.43.1-stage60c` (version code 63) connects the registered `com.dolo.patient` Android client to Firebase Cloud Messaging without committing its Firebase configuration. GitHub Actions reconstructs `google-services.json` from the protected repository secret and verifies the package before building. The App requests Android notification permission only when the Patient saves enabled Push consent, creates a dedicated Appointment updates channel, displays only fixed generic notification text, and accepts only bounded appointment routes. A prominent denial notice confirms that login, booking, queue tracking and local data continue normally and points to Android App info for later enablement. It stores only a SHA-256 token fingerprint locally and never logs or displays the raw token.
...
## Stage 62E implementation checkpoint

Patient App `0.48.0-stage62e` (version code 69) removes the Home hero artwork and displays a saved Male/Female Patient avatar in a white circle next to the name. Profile & family now persists the avatar choice locally; older installations default safely to Male. Favourite Doctor actions and Appointments filter selections explicitly use the blue theme instead of inherited green/teal roles.

Both local Doctor Details and hosted Doctor Profile now include the shared Home, Book and Appointments navigation. Their titles use the active background-content color and remain visible in both themes. Daylight cards and fields are flat and borderless until focused, while Night mode keeps visible layered surfaces. Existing application data and business workflows are unchanged. GitHub Actions run 31502904923 passed compile, lint, unit tests, signing verification and stable APK generation. Install that artifact over the accepted App and follow `docs/stage62e-device-test.md`.