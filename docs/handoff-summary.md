...
Both local Doctor Details and hosted Doctor Profile now include the shared Home, Book and Appointments navigation. Their titles use the active background-content color and remain visible in both themes. Daylight cards and fields are flat and borderless until focused, while Night mode keeps visible layered surfaces. Existing application data and business workflows are unchanged. Build through GitHub Actions and follow `docs/stage62e-device-test.md`.